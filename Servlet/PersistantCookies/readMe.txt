This program is all about How some websites like facebook remember you even after you closed browser ?
...
Means, when the session object gets created at server side. it has a unique id (you can use session.getId() to check id). when you set attributes in that session, all those attributes are stored at serverside.  we DO NOT include this attributes in response. we just give that unique id(JESSSIONID) in the response. This is clear indication to the client(browser) that, "please include this id in the future requests also". Now this id will not get stored in browser. so when you close the browser, now the browser do not have that unique id given by server.
...
1) when session is created , we use a HashMap to store this session. 
	HashMap<key,value> : key = unique id , value : HttpSesssion object. 
...
for more : http://stackoverflow.com/questions/16099752/session-or-cookie-confusion
